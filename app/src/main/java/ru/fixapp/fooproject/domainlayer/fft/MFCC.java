...
public class MFCC {
	...
	public MFCC(int fftSize, int numCoeffs, int melBands, double sampleRate) {
		...
		for (int i = 0; i < melBands + 2; i++) {
			binFreqs[i] =
					fmel2hz(minMel + (double) i / ((double) melBands + 1.0) * (maxMel - minMel));
		}
		...
		for (int i = 0; i < melBands; i++) {
			for (int j = 0; j < fftSize; j++) {
				...
				double hiSlope =
						(binFreqs[i + 2] - fftFreqs[j]) / (binFreqs[i + 2] - binFreqs[i + 1]);
		...
		for (int i = 0; i < numCoeffs; i++) {
			for (int j = 0; j < melBands; j++) {
				double phase = j * 2 + 1;
				dctMat.A[i][j] =
						Math.cos((double) i * phase / (2.0 * (double) melBands) * Math.PI) * scale;
			}
		}
